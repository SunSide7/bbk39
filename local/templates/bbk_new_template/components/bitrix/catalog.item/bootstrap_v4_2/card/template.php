...
<!--                </div>-->
    ...
    <div class="for_designers_card_text">
        ...
        <?
        $filePath = CFile::GetPath($item['PROPERTIES']['DESIGNER_PROJECT_FILES']['VALUE'])
        ?>

        <? if($USER->IsAuthorized()) { ?>
        <a download style="<?= $filePath ? '' : 'opacity: 0.5;cursor:initial;' ?>" href="<?= $filePath ?>" class="btn_blue">
            Скачать текстуры
        </a>
        <? } else { ?>
            <a data-toggle="modal" data-target="#authModal" class="btn_blue" style="color: #fff;cursor: pointer;">
                Скачать текстуры
            </a>
        <? } ?>

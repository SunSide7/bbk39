    ...
    <? if(false) { ?>
        <?php foreach ($arResult['SECTIONS'] as $idx => $arSection): ?>
            ...
            <? if ($arSection['RELATIVE_DEPTH_LEVEL'] == 1) { ?>
                    ...
                    <? if(
    //                        $APPLICATION->GetCurPage(false) === $arSection['SECTION_PAGE_URL']
                            str_contains( $APPLICATION->GetCurPage(false), $arSection['SECTION_PAGE_URL']) ||
                            str_contains( $APPLICATION->GetCurPage(false), $arParams['SECTION_URL_ATT'])
                    ) { ?>
